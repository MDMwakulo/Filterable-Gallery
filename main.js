window.addEventListener('DOMContentLoaded', () => {
	...
	const filterImages = (event) => {
		...
		document.querySelectorAll('.gallery-image').forEach((element) => {
			if (event.target.id !== element.dataset.category && event.target.id !== 'all') {
	...
	images.forEach((element) => {
		...
		domElement.dataset.category = element.category; //creating custom data attributes we use dataset then name of the attribute.
